/*
 * Conway's Game of Life, the workload Oracle uses to document PGO gains in
 * docs/reference-manual/native-image/PGO-Basic-Usage.md, reproduced verbatim from that document so
 * that CrucibleVM can be measured on the same program rather than only on its own benchmarks.
 *
 * Imports are added here because the documentation lists them separately from the class body.
 */
